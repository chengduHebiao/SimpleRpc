      ...
      $("[name='datali']>div").click(function () {
        if (!opts.isShowCheckBox) {
          ...
          if ($(this).children('input[type="checkbox"]').is(":checked")) {
          	if(!$(this).children('input[type="checkbox"]').hasClass('child')){
          	$(this).next().children('li').find('div').children('input[type="checkbox"]').prop('checked',false);
          	}
            $(this).children('input[type="checkbox"]').prop("checked", false);
          } else {
            $(this).children('input[type="checkbox"]').prop("checked", true);
            if($(this).children('input[type="checkbox"]').hasClass('child')){
              if( !$(this).parent().parent().prev().children('input[type="checkbox"]').is(":checked")){
...
*/
          $('#' + id).attr('data-id', data_id).val(data_value);
          $('#' + id + "_hcd").remove();
          $('#' + id).removeClass('hsCjeckData_check');
        });
      }
    ...
    function setShowData() {
      if ($('#' + id).attr("data-id") != undefined && $('#' + id).attr("data-id") != "") {
        ...
        var text = "";
        for (var i = 0; i < dataidArray.length; i++) {
          var name = getName(opts.data, dataidArray[i]);
          text += (name==undefined ? '': name +",");
        }
        $('#' + id).val(text.substr(0, text.length - 1));
      }
    }
    //获取名称
    ...
    function childDataHtml(json) {
      ...
      for (var i = 0; i < json.length; i++) {
        for (var key in json[i]) {
          html += "<li name='datali'><div>";
          if (opts.isShowCheckBox) {
            html += "<input name='childcheckbox' class='child' type='checkbox' value='" + key + "'/>";
          } else {
            html += "<input name='childcheckbox' class='child' style='display:none' type='checkbox' value='" + key + "'/>";
          }
      ...
      for (var i = 0; i < json.length; i++) {
        for (var key in json[i]) {
          if (json[i][key].indexOf(filterInput) > -1) {
            html += "<li name='datali'><div>";
            if (opts.isShowCheckBox) {
              html += "<input name= 'childcheckbox' class='child' name= 'dataliCheck' type= 'checkbox' value= '" + key + "' />";
            } else {
              html += "<input name= 'childcheckbox' class='child' name= 'dataliCheck' style='display:none' type= 'checkbox' value= '" + key + "' />";
            }
            html += "<span>" + json[i][key] + "</span></div ></li > ";
          }
        }
